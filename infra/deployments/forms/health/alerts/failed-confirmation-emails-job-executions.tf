resource "aws_cloudwatch_metric_alarm" "failed_confirmation_emails_job_executions" {
  alarm_name          = "${var.environment}-failed-confirmation-emails-job-executions"
  alarm_description   = <<EOF
There are one or more failed job executions for the confirmation_emails queue in forms-runner in the ${var.environment}
